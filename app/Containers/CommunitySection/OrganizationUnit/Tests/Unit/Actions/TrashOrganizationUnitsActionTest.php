...
final class TrashOrganizationUnitsActionTest extends UnitTestCase
{
    public function testSuccess(): void
    {
        ...
        $result = app(TrashOrganizationUnitsAction::class)->run($ids->toArray());

        $this->assertSame($models->count(), $result);

        $models
            ->each(function (OrganizationUnitModel $model) {
                $model->refresh();
                $this->assertSoftDeleted(OrganizationUnitModel::TABLE, [ID => $model->id]);
            });
    }
}
