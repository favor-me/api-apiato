<?php

/**
 * FavorMe system
 *
 * This file is part of the FavorMe system package.
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 *
 * @license https://favor-me.ru/licenses/erp Proprietary license
 * @copyright Copyright (C) kalistratov.ru, All rights reserved ©.
 * @link https://kalistratov.ru
 * @author Sergey Kalistratov <sergey@kalistratov.ru>
 */

namespace App\Containers\AccountingSection\Contract\Tests\Unit\Actions;

use App\Containers\AccountingSection\Contract\Actions\DeleteContractsAction;
use App\Containers\AccountingSection\Contract\Models\Contract as ContractModel;
use App\Containers\AccountingSection\Contract\Tests\UnitTestCase;

final class DeleteContractsActionTest extends UnitTestCase
{
    public function testNotTrashed(): void
    {
        $this->getTestingOrganizationUser();

        $model = ContractModel::factory()
            ->counterparty($this->testingUser->organization_id)
            ->create();

        $result = app(DeleteContractsAction::class)->run([$model->id]);

        $this->assertSame(ZERO, $result);
    }

    public function testTrashed(): void
    {
        $this->getTestingOrganizationUser();

        $models = ContractModel::factory()
            ->count(2)
            ->trashed()
            ->counterparty($this->testingUser->organization_id)
            ->create();

        $ids = $models
            ->pluck(ID)
            ->toArray();

        $result = app(DeleteContractsAction::class)->run($ids);

        $this->assertSame($models->count(), $result);
    }
}
